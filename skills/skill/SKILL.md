...
2. Scan user skills at `~/.gemini/antigravity/skills/omc-learned/`
...
USER SKILLS (~/.gemini/antigravity/skills/omc-learned/):
...
5. **Ask for scope:**
   - `user` → `~/.gemini/antigravity/skills/omc-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.gemini/antigravity/skills/omc-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `~/.gemini/antigravity/skills/omc-learned/<name>/SKILL.md`
   - `.omc/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `~/.gemini/antigravity/skills/omc-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.gemini/antigravity/skills/omc-learned/)
...
```
Skill: custom-logger
Scope: user (~/.gemini/antigravity/skills/omc-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.gemini/antigravity/skills/omc-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.gemini/antigravity/skills/omc-learned/custom-logger/SKILL.md
...
USER_SKILLS_DIR="$HOME/.gemini/antigravity/skills/omc-learned"
...
echo "=== USER-LEVEL SKILLS (~/.gemini/antigravity/skills/omc-learned/) ==="
if [ -d "$HOME/.gemini/antigravity/skills/omc-learned" ]; then
  USER_COUNT=$(find "$HOME/.gemini/antigravity/skills/omc-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "$HOME/.gemini/antigravity/skills/omc-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Then ask for scope:
- **User-level** (~/.gemini/antigravity/skills/omc-learned/) - Available across all projects
...
✓ User skills directory exists: ~/.gemini/antigravity/skills/omc-learned/
